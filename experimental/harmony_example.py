...
model = AutoModelForCausalLM.from_pretrained(
    model_id,
    device_map="cuda",  # automatically place on available CPU/GPU(s)
    torch_dtype="auto",
)
...
python_tool = PythonTool()  # 参考实现（需要本机 Docker 可用）
...
system_msg = (
    ...
    .with_tools(python_tool.tool_config)   # ✅ 注入命名空间对象
    .with_tools(browser_tool.tool_config)  # ✅ 注入命名空间对象
)
